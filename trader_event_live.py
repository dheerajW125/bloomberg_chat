...
def load_symbols(
    ...
    if suffix == ".csv":
        with path.open("r", encoding="utf-8-sig", newline="") as handle:
            rows = list(csv.reader(handle))[skip_rows:]
        if not rows:
...
def main() -> int:
    ...
    parser.add_argument(
        "--symbol-sheet",
        default=0,
        help="Excel sheet name or zero-based index",
    )
                ...
                if args.follow:
                    if matcher:
                        ...
                        for pending in pending_events:
                            client_intent, basis, delta = matcher.match(pending)
                            if client_intent is None or delta is None:
                                still_pending.append(pending)
                                continue
                            resolved = mapped_event(
                                pending,
                                client_intent,
                                basis,
                                delta,
                            )
                            resolved["mapping_status"] = "matched"
                            ...
                            target.flush()
                            print(
                                f'{resolved["event_type"]} '
                                f'trader={resolved["trader_id"]} '
                                f'client={resolved["client_id"]} '
                                f'room={resolved["room_id"]} '
                                f'(resolved pending mapping)',
                                flush=True,
                            )
                ...
                if matcher:
                    ...
                    if client_intent is None or delta is None:
                        if args.follow:
                            ...
                            print(
                                f'PENDING {normalized["event_type"]} '
